...
max_iter = 20
dist_func = dtw2
#centers_temp = get_center_lane(highway)
trajs = init_trajs(highway)
#test
membership, centers, total_cost = clustering1(trajs_ene,n_clusters,max_iter, dist_func)
#######
...
membership1,centers1,cost1 = run_r_times_cluster1(r,trajs,n_clusters,max_iter,dist_func)
plot_trajectoires(trajs,membership1,centers1,'cluster_method1.png')
...
trajs = init_trajs(highway)
#test
membership2,centers2,total2 = clustering2(trajs,n_clusters,max_iter, dist_func,h)
...
membership2,centers2,total2 = run_r_times_cluster2(r,trajs,n_clusters,max_iter,dist_func,h)
plot_trajectoires(trajs,membership2,centers2,'cluster_method2.png')

#####5.2.2 using energy.csv
energy = pd.read_csv('energy.csv')

trajs_ene = init_trajs(energy)

#get the cost as a function of k using method1, k is from 1 to 16.
costs = []
for k in range(1,17):
    max_iter = 20
    dist_func = dtw2
    r = 10
    mem_k,centers_k,cost_k = run_r_times_cluster1(r, trajs_ene,k, max_iter, dist_func)
    costs.append(cost_k)

#For k=8,get the clustering using method 1
r = 10
max_iter = 20
n_clusters = 8
dist_func = dtw2
membership1,centers1,cost1 = run_r_times_cluster1(r,trajs_ene,n_clusters,max_iter,dist_func)
plot_trajectoires(trajs_ene,membership1,centers1,'energy_method1.png')
print(cost1)
costs_copy = costs
##get the cost as a function of k using method2, k is from 1 to 16.
costs2 = []
for k in range(1,17):
    max_iter = 20
    dist_func = dtw2
    r = 10
    h = 6
    mem_k,centers_k,cost_k = run_r_times_cluster2(r, trajs_ene,k, max_iter, dist_func,h)
    costs2.append(cost_k)

#k=8,get the clustering using method 1
r = 10
max_iter = 20
n_clusters = 8
dist_func = dtw2
h = 6
membership2,centers2,total2 = run_r_times_cluster2(r,trajs_ene,n_clusters,max_iter,dist_func,h)
plot_trajectoires(trajs_ene,membership2,centers2,'energy_method2.png')
print(total2)
...
x = list(range(1,17))
y1 = costs
plt.plot(x, y1,label='first center computation')
...
x = list(range(1,17))
y2 = costs2
plt.plot(x, y2,label='second center computation')
plt.xlabel('Number of clusters(k)')
plt.ylabel('Cost')
plt.legend()
